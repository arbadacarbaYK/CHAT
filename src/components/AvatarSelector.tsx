    ...
    <div className="flex flex-col items-center justify-center min-h-screen bg-gradient-to-br from-orange-100 to-yellow-200 p-4">
      ...
      <div className="flex space-x-8 mb-8">
        {avatars.map((avatar) => (
          <button
            key={avatar.label}
            className={`flex flex-col items-center p-4 rounded-lg border-2 transition-all duration-200 shadow-lg bg-white hover:border-orange-400 focus:outline-none ${selectedAvatar.label === avatar.label ? 'border-orange-500' : 'border-gray-200'}`}
